...
再次运行：没有任何输出，进程以状态码 0 退出——但失败并没有被吞掉。`Error: apply exploded` 经 Cordis logger 服务记录，该插件的 fiber 进入 FAILED 状态，插件永远不会进入 ACTIVE。loader 按配置项隔离失败：既不会装作加载成功而跳过它，也不会让进程崩溃。

那么错误报告去哪了？logger 服务的输出去向由**导出器（exporter）**决定，而本教程的启动器没有挂载任何控制台导出器，所有报告只留在内存缓冲区中。即使按[第 6 章](06-composition-and-hmr.zh.md)的方式通过 `cordis.yml` 挂载 `@deepseek-ai/cordis-plugin-logger-console`，启动期的报告也通常先于导出器完成注册而丢失；导出器就绪后发生的错误（如第 6 章 HMR 触发的重载）才会打印到控制台。

如果某个配置项的模块无法被**解析**——例如路径或包名拼写错误——走的也是同一条静默路径：错误被记录（同样只进缓冲区）、该配置项不会加载、进程退出 0。如果新增配置项似乎没有任何效果，请先检查拼写。
